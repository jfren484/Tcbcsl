                        ...
                        Schedule = (from gp in ty.GameParticipants
                                    ...
                                    select new TeamGameModel
                                    {
                                        ...
                                        OpponentName = opponent.TeamYear.Church.DisplayName
                                                       + (string.IsNullOrEmpty(opponent.TeamYear.TeamName) ? string.Empty : " " + opponent.TeamYear.TeamName)
                                                       + (gp.Game.GameTypeId == GameType.Exhibition ? " *" : string.Empty),
                                        IsGameCompleted = gp.Game.GameStatus.IsComplete,
                                        DidWin = won,
                                        DidLose = lost,
                                        IsHomeTeam = gp.IsHost,
                                        IsNeutralSite = gp.Game.GameTypeId == GameType.GamePlaceholder || gp.Game.GameTypeId == GameType.PostSeason,
                                        ...
                                        GameResultDescription = gp.Game.GameStatus.IsComplete
                                            ? winLossChar + " " + SqlFunctions.StringConvert((decimal)winnerRuns) + "-" + SqlFunctions.StringConvert((decimal)loserRuns)
                                            : gp.Game.GameStatusId != GameStatus.Scheduled ? gp.Game.GameStatus.Description : string.Empty
                                    })
                            .ToList()
